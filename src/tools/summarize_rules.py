...
class Summarizer:
    ...
    def split_rules_list(self, rule_list, token_limit, model='gpt-4-turbo'):
        """
        ...
        for rule in rule_list:
            rule += '\n'
            rule_token_count = self.num_tokens_from_message(rule, model) - 4
    ...
    def split_response_list(self, content_list, summarize_prompt, model='gpt-4-turbo'):
        token_limitation = self.get_token_limit(model)  # Get input token limitation for current model
        all_rules_content = '\n'.join(content_list)
        formatted_all_response = summarize_prompt.format_map({'instruction': all_rules_content.strip()})
        token_num_all_response = self.num_tokens_from_message(formatted_all_response, model)
        if token_num_all_response > token_limitation:
            current_len = self.num_tokens_from_message(summarize_prompt,model)
            token_limitation -= current_len
            list_of_response = self.split_rules_list(content_list, token_limitation, model)
        else:
    ...
    def run(self):
        ...
        rule_file_list = self.load_rule_file(input_file_path)
        summarize_prompt = (
            ...
            "\n{instruction}")
        splitted_response_list = self.split_response_list(rule_file_list, summarize_prompt)
        response_list = []
        for rule_list in splitted_response_list:
            rule_content = '\n'.join(rule_list)
            input_content = summarize_prompt.format_map({'instruction': rule_content.strip()})
            response = self.query(input_content, args['api_key'], model='gpt-4-turbo')
    ...
    def __init__(self, args=None, model_folder=None, dataset=None, subtask=None, api_key=None):
        if args is None: 
        ...
        if isinstance(args, dict): 
            if model_folder is None: args['input_model_folder'] = 'chemfdm'
            elif model_folder not in ('chemfdm', 'chemllm-7b', 'falcon-7b', 'falcon-40b', 'galactica-6.7b', 'galactica-30b'):
                raise Exception('Invalid model..')
            else: args['input_model_folder'] = model_folder
            ...
            else: raise Exception('No API_KEY')

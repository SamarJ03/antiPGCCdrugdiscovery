...
class Inference:
    ...
    def get_system_prompt():
        ...
        if model in ['falcon-7b', 'falcon-40b']:
            ...
            system_prompt = ("Below is an instruction that describes a task. "
                            ...
                            "### Instruction:n{instruction}nn### Response:n")
        elif model in ["chemdfm"]:
            system_prompt = "Human: {instruction}nAssistant:"
        else:
    ...
    def get_model_response(self, model, list_of_smile_label_lists, pipeline, dk_prompt, tokenizer):
        input_list = [dk_prompt + 'n' + 'n'.join(batch) for batch in list_of_smile_label_lists]
        system_prompt = self.get_system_prompt()
        response_list = []
        model = model.lower()
        supported_models = ['falcon-7b', 'falcon-40b', 'galactica-6.7b', 'galactica-30b', 'chemllm-7b', 'chemdfm']
        if model not in supported_models:
            raise NotImplementedError(f"get_model_response() is not implemented for model {model}.")

        for input_text in input_list:
            full_prompt = system_prompt.format_map({'instruction': input_text.strip()})
            # print(full_prompt)  # for debugging

            prompt_token_len = len(tokenizer.tokenize(full_prompt))
            max_new_token = self.get_token_limit(model, for_response=True) - prompt_token_len

            if model == 'chemllm-7b':
                inputs = tokenizer(full_prompt, return_tensors="pt").to("cuda")
                config = GenerationConfig(
                    do_sample=True, top_k=1, temperature=0.5,
                    max_new_tokens=max_new_token, repetition_penalty=1.2,
                    pad_token_id=tokenizer.eos_token_id
                )
                outputs = pipeline.generate(**inputs, generation_config=config)
                generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
            elif model == 'chemdfm':
                inputs = tokenizer(full_prompt, return_tensors="pt").to("cuda")
                config = GenerationConfig(
                    do_sample=True, top_k=20, top_p=0.9, temperature=0.9,
                    max_new_tokens=max_new_token, repetition_penalty=1.05,
                    pad_token_id=tokenizer.eos_token_id
                )
                outputs = pipeline.generate(**inputs, generation_config=config)
                generated_text = tokenizer.batch_decode(outputs, skip_special_tokens=True)[0][len(full_prompt):]
            else:
            ...
            if model in ["galactica-6.7b", "galactica-30b", "chemllm-7b"]:
                parts = generated_text.split('### Response:n')
                generated_text = parts[1].strip() if len(parts) > 1 else generated_text.strip()
            else:
    ...
    def run(self, args):
        if args.dataset in [rdkitDescName for rdkitDescName, fxn in Descriptors.descList]:
        ...
        if args.subtask:
            ...
            if args.dataset in [rdkitDescName for rdkitDescName, _ in Descriptors.descList]:
